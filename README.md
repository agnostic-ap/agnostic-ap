<h1 align="center">Aldrin · agnostic-ap</h1>

<p align="center">
  <strong>AI-native full-stack developer — I ship complete products solo, front to back.</strong>
</p>

<p align="center">
  <a href="https://github.com/agnostic-ap"><img src="https://img.shields.io/badge/GitHub-agnostic--ap-1a1b27?style=for-the-badge&logo=github&logoColor=70A5FD" alt="GitHub" /></a>
  <a href="https://gt2kk.cn"><img src="https://img.shields.io/badge/Website-gt2kk.cn-1a1b27?style=for-the-badge&logo=vercel&logoColor=70A5FD" alt="Website" /></a>
  <a href="mailto:dc23byte@gmail.com"><img src="https://img.shields.io/badge/Email-dc23byte%40gmail.com-1a1b27?style=for-the-badge&logo=gmail&logoColor=70A5FD" alt="Email" /></a>
</p>

---

## What I do

I take a rough product idea and ship it end to end — frontend architecture, backend and database, deployment, ops, and iteration after launch. Alone. Fast. Increasingly with an AI-native workflow (Claude Code / Codex) that lets one person move like a small team.

5 years across production web apps, WeChat Mini Programs, native Android, and 100+ commercial automation scripts — for New Zealand clients and my own products.

## Products I've shipped 🚀

Real, live, self-hosted software — not tutorials or forks.

| Product | Live | Stack | What it solves |
|---|---|---|---|
| **江苏自考帮** | [jszkbang.cn](https://jszkbang.cn) | Next.js · Fastify · Drizzle · PostgreSQL · Docker | One-stop exam-prep platform for Jiangsu self-study candidates: major planning, question bank, review, flashcards, AI explanations |
| **MD2Card** | [mdcard.cn](https://mdcard.cn) | React · Vite · Fastify · SQLite | Turn Markdown into beautiful shareable knowledge-card images — live preview, themes, AI generation, one-click export, self-built analytics + admin |
| **简历工作室** | [jianlistudio.cn](https://jianlistudio.cn) | Vue 3 · Vite · Ant Design | Online resume builder — editor, admin dashboard, backend API, multi-repo workspace with shared-config automation |
| **逻辑支点** | [lg.mdcard.cn](https://lg.mdcard.cn) · WeChat Mini Program | Taro · React · Bun · Hono · SQLite | Investment decision journal — log the reasoning behind every trade, replay context, review outcomes |

Each one is the full stack in one pair of hands: I designed the product, wrote the frontend and backend, set up the database, and deployed and now run it in production (Caddy + PM2 + Docker on my own server).

## Toolbox

**Frontend** Vue 2/3 · React · Next.js · Nuxt · Taro · TypeScript · Tailwind CSS
**Backend** Fastify · Hono · Bun · Node · Laravel · Spring Boot · REST · OAuth2
**Data & Infra** PostgreSQL · SQLite · Redis · Drizzle · Docker · Caddy · PM2 · Linux VPS
**Mobile** Kotlin (Android) · Uniapp · WeChat Mini Programs · TRTC · Tencent IM
**AI** Claude Code / Codex agent workflows · OpenAI API · 通义千问

---

<p align="center"><em>Featured products are pinned below. Reach me at dc23byte@gmail.com — open to remote work.</em></p>
